...
int  hasCycle(struct vertex *src,struct vertex *parent){
	...
	while(el!=NULL){
		...
		if(el->v->visited==0){
			parent=src;
			printf("elData:%c, parent:%c \n",el->v->data,parent->data);
			if(hasCycle(el->v,parent)==1){
				return 1;
			}
